...
const Project = () => {
    ...
    useEffect(()=>{
        ...
        axios.get('http://localhost:5000/getstudentproject',{
            ...
            !search ? setStudentprojects(res.data) : setStudentprojects(res.data.filter(profile => profile.clgid.includes(search.toUpperCase()) | profile.projecttitle.toLowerCase().includes(search.toLowerCase()) | profile.name.toLowerCase().includes(search.toLowerCase()))))
